...
final class DataStore: ObservableObject {
    ...
    private var windowUndoManager: UndoManager? { NSApp.mainWindow?.undoManager }
    ...
    func addProject(_ project: Project) {
        let id = project.id
        windowUndoManager?.registerUndo(withTarget: self) { target in
            target.deleteProject(id: id)
        }
        windowUndoManager?.setActionName("新建项目")
    ...
    func deleteProject(id: String) {
        ...
        windowUndoManager?.setActionName("删除项目")
    ...
    func updateProject(_ project: Project) {
        ...
        windowUndoManager?.setActionName("修改项目")
    ...
    func addLogEntry(date: String, entry: LogEntry) {
        ...
        windowUndoManager?.setActionName("添加工作记录")
    ...
    func deleteLogEntry(date: String, entryId: String) {
        ...
        windowUndoManager?.setActionName("删除工作记录")
    ...
    func updateLogEntry(date: String, entry: LogEntry) {
        if let dIdx = dailyLogs.firstIndex(where: { $0.date == date }),
           let eIdx = dailyLogs[dIdx].entries.firstIndex(where: { $0.id == entry.id }) {
            ...
            windowUndoManager?.setActionName("修改工作记录")
