...
- 标准月历网格布局
- 有工作记录的日期带颜色标记
- 每个日期格子内显示当天关联的项目名和简要摘要（截断显示）
- 支持切换月份

#### 4. 日详情

- 点击月视图中某一天，展开或跳转到日详情视图
...
使用 `DispatchSource.makeFileSystemObjectSource` 监听数据目录：

- 监听 `projects.json`、`daily-logs.json` 的修改事件
- 监听 `drafts/` 目录的文件创建、修改、删除事件
...
| `FileWatcher` | 封装 DispatchSource，监听文件变化，回调通知 DataStore |
