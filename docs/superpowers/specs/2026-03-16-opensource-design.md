...
- `docs/superpowers/` 目录：Claude Code 内部设计文档，不应出现在公开仓库
  - 该目录已有提交历史，需执行 `git rm -r --cached docs/superpowers/` 将其从 Git 跟踪中移除，然后提交删除记录
  - 同时在根目录 `.gitignore` 中加入 `docs/superpowers/` 防止误提交

### 保留内容

- 仓库根目录 `CLAUDE.md`：项目说明，无敏感信息，对使用 Claude Code 的贡献者有用
