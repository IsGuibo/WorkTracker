...
| `.gitignore` | 仓库根目录 | 排除 `.DS_Store`、`.claude/`、`CLAUDE.md`、`docs/superpowers/` |

### 删除内容（均已有提交历史，需用 `git rm --cached` 移除跟踪）

- `docs/superpowers/` 目录：Claude Code 内部设计文档
  - `git rm -r --cached docs/superpowers/`
- `CLAUDE.md`：Claude Code 专用项目指令，属内部工具文件
  - `git rm --cached CLAUDE.md`

两者均在根目录 `.gitignore` 中加入对应规则，防止误提交。

### 保留内容

- `WorkTracker/claude-md-template.md`：数据格式文档，内容对用户有参考价值
- `WorkTracker/build_app.sh`：已存在的打包脚本，README 安装说明会引用
...
## Screenshots / 截图
（本次范围内暂不处理，截图占位节保留，后续由作者自行补充到 docs/screenshots/ 目录）
